...
SELECT
    ...
    sum(DISTINCT CASE WHEN order_c.status_id=1 THEN order_c.total_sum END) "Сумма выполенных заказов",
    avg(DISTINCT CASE WHEN order_c.status_id=1 THEN order_c.total_sum END)::decimal(10,2) "Средняя стомость выполненного заказа"
FROM
    order_c
    JOIN
    order_diff
